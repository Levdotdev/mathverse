...
@php
    ...
    $healthCards = [
        ...
        ['key' => 'queue', 'title' => 'Laravel Queue', 'icon' => 'fa-layer-group', 'metric' => ($health['queue']['failed'] ?? '—').' failed'],
...
<div class="max-w-7xl mx-auto" data-testid="system-health" data-overall-status="{{ $health['overall'] }}">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-6 text-[10px] text-slate-500"><span><i class="fas fa-code-commit mr-2"></i>Commit <code class="text-slate-300">{{ $health['commit'] }}</code></span><span>Checked {{ \App\Support\AppDate::relative($health['generated_at']) }} · <a href="/admin/system-health" class="text-cyan-400">Run checks again</a></span></div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <section class="portal-frame !p-5 md:!p-6" aria-labelledby="scheduler-health-title"><h3 id="scheduler-health-title" class="font-orbitron font-bold mb-4">Scheduler and Queue</h3><dl class="health-detail-list"><div><dt>Scheduler last seen</dt><dd>{{ \App\Support\AppDate::format($health['scheduler']['last_seen_at'], 'M j, Y g:i:s A', 'Never') }}</dd></div><div><dt>Queue connection / driver</dt><dd>{{ $health['queue']['connection'] }} / {{ $health['queue']['driver'] }}</dd></div><div><dt>Queued jobs</dt><dd>{{ $health['queue']['queued'] ?? ($health['queue']['requires_worker'] ? 'Unavailable' : 'Not applicable') }}</dd></div><div><dt>Recorded failed jobs</dt><dd>{{ $health['queue']['failed'] ?? ($health['queue']['requires_worker'] ? 'Check configured failure storage' : 'See application logs') }}</dd></div></dl><p class="text-[10px] text-slate-500 mt-4"><code>php artisan schedule:run</code> must run every minute. {{ $health['queue']['requires_worker'] ? 'The active queue requires a working queue worker.' : 'This queue driver does not require a queue worker or local queue tables.' }}</p></section>
    </div>
</div>
@endsection
