...
<div class="max-w-7xl mx-auto" data-testid="system-health" data-overall-status="{{ $health['overall'] }}">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-6 text-[10px] text-slate-500"><span><i class="fas fa-code-commit mr-2"></i>Commit <code class="text-slate-300">{{ $health['commit'] }}</code></span><span>Checked {{ \Carbon\Carbon::parse($health['generated_at'])->diffForHumans() }} · <a href="/admin/system-health" class="text-cyan-400">Run checks again</a></span></div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <section class="portal-frame !p-5 md:!p-6" aria-labelledby="scheduler-health-title"><h3 id="scheduler-health-title" class="font-orbitron font-bold mb-4">Scheduler and Worker</h3><dl class="health-detail-list"><div><dt>Scheduler last seen</dt><dd>{{ $health['scheduler']['last_seen_at'] ? \Carbon\Carbon::parse($health['scheduler']['last_seen_at'])->timezone(config('app.timezone'))->format('M j, Y g:i:s A') : 'Never' }}</dd></div><div><dt>Queued jobs</dt><dd>{{ $health['queue']['queued'] ?? 'Unavailable' }}</dd></div><div><dt>Failed jobs</dt><dd>{{ $health['queue']['failed'] ?? 'Unavailable' }}</dd></div></dl><p class="text-[10px] text-slate-500 mt-4"><code>php artisan schedule:run</code> must run every minute. A queue worker is also required when the configured queue is asynchronous.</p></section>
    </div>
</div>
@endsection
