...
function loadQuizBuilder(quizId = null) {
    ...
    if (quizId) {
        ...
        // Fetch existing quiz data to pre-fill the form
        fetch(`/teacher/quiz/${quizId}/results`)   // re-use results endpoint just for session data
            .then(() => {});

        // Pull quiz details from the already-rendered page data
        const card = document.querySelector(`[data-quiz-id="${quizId}"]`);
        if (card) {
            document.getElementById('q-topic').value       = card.dataset.topic;
            document.getElementById('q-room-code').value   = card.dataset.code;
            document.getElementById('q-max-members').value = card.dataset.max;
            document.getElementById('q-class').value       = card.dataset.classId || '';
        }
    } else {
...
function addQuestionBlock(num, text, opts, correctIndex) {
    ...
    const idx       = container.children.length;  // use as array index for name attributes
    ...
    div.innerHTML   = `
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
            ...
            <div class="relative">
                ...
                <input type="text" name="questions[${idx}][options][]" value="${opts[i] || ''}"
                       placeholder="Option ${i+1}" class="input-mobile-ultra">
            </div>`).join('')}
        </div>
        <div class="form-group">
            <label class="input-label text-green-400">Correct Answer</label>
            <select name="questions[${idx}][correct]" class="input-mobile-ultra bg-slate-900 border-green-500/30 text-green-400">
                ${[0,1,2,3].map(i =>
...
// Pre-fill builder when editing (data embedded via data attributes on quiz cards)
document.querySelectorAll('[data-quiz-id]').forEach(card => {
    // Data attributes set in Blade for JS access
});
...
async function openResultsModal(sessionId, topic) {
    ...
    if (!data.length) {
        tbody.innerHTML = '<tr><td colspan="4" class="text-center py-6 text-slate-500 text-xs uppercase">No attempts recorded yet.</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = data.map(r => {
        ...
        <tr class="border-b border-white/5 hover:bg-white/5">
            ...
            <td class="py-4 text-slate-500 text-xs">${date}</td>
        </tr>`;
    }).join('');
}
        ...
        <tr class="border-b border-white/5 hover:bg-white/5">
            ...
            <td class="py-4 text-right">
                <button onclick="removeStudent('${s.id}', '${classId}')"
                        class="text-red-500 hover:text-white transition-all text-[10px] font-bold uppercase">
...
async function fetchLobbyParticipants() {
    ...
    if (!data.length) {
        tbody.innerHTML = '<tr><td class="p-6 text-center text-slate-500 text-xs uppercase tracking-widest">Awaiting connections...</td></tr>';
        return;
    }
...
async function endQuiz() {
    if (!confirm('Terminate the quiz for all students?')) return;
...
// Auto-open section if redirected with ?section= param
